...
export const useServiceCategories = () => {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      ...
      console.log(
        "useServiceCategories: Is array?",
        Array.isArray(rootChildren)
      );

      if (!Array.isArray(rootChildren)) {
        console.log(
          "useServiceCategories: rootChildren is not an array, setting empty"
        );
        setCategories([]);
        return;
      }
      ...
      const servicesCategories = rootChildren.filter((item) => {
        ...
        console.log(
          `useServiceCategories: Item ${item.id}: isStatic=${isStatic}, isDynamic=${isDynamic}, kind=${item.kind}`
        );
      ...
      console.log(
        "useServiceCategories: Final categories:",
        servicesCategories
      );
      setCategories(servicesCategories);
    } catch (err) {
      // Если 404 - просто возвращаем пустой массив без ошибки
      if (err.status === 404) {
        console.warn("useServiceCategories: 404 - services not found in DB");
        setCategories([]);
        setError(null);
      } else {
...
export const useServiceCategoryDetail = (slugPath = [], lang = "ua") => {
  ...
  const fetchCategory = useCallback(async () => {
    ...
    try {
      ...
      // Если 404 - возвращаем заглушку с текстом
      if (err.status === 404) {
        console.warn(
          "useServiceCategoryDetail: 404 - category not found in DB"
        );
        setCategory({
          label: { ua: "Сторінка", ru: "Страница", en: "Page" },
