...
const normalizedHtml = useMemo(() => normalizeEditorHtml(content), [content]);
  ...
  // Извлекаем параграфы из контента
  const paragraphs = useMemo(() => {
    if (!content) return [];
    return extractParagraphsFromHTML(content);
  }, [content]);

  // Определяем, нужно ли показывать кнопку (больше 2 параграфов)
  const shouldShowButton = paragraphs.length > 2;

  // Получаем контент для отображения
  const displayContent = useMemo(() => {
    if (!shouldShowButton || expanded) {
      return normalizedHtml;
    }
    
    // Показываем только первые 2 параграфа
    const firstTwoParagraphs = paragraphs.slice(0, 2);
    return firstTwoParagraphs.map(p => `<p>${p}</p>`).join('');
  }, [normalizedHtml, paragraphs, expanded, shouldShowButton]);
  ...
  return (
    <div className="about-container">
      <div className="container container-1200">
        ...
        {shouldShowButton && (
          <button
            className={`accordion-toggle uppercase main-text-toggle-btn ${isPC ? "fs-p--16px" : "fs-p--14px"}`}
